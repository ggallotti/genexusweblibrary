		...
		function table() {
			...
			function fixLeft() {
				...
				tr.each(function (k, row) {
					...
					// for(var colsRight = 1; colsRight <= fixColumn; colsRight = colsRight + inc) {
					// 	var nth = inc > 1 ? colsRight - 1 : colsRight;
...
WebGridExtension = (function () {
	...
	return {
		freezeColumnsAdvanced: function (gridName, colsLeft, colsRight, width, height) {	
			var gxO = gx.O,
			...
			$gridTable.width($gridTable.width());
			width && $gridContainerDiv.css({
				width: width
			});
			height && $gridTable.height() > height && $gridContainerDiv.css({
				height: height
			});
			$gridContainerDiv.css("overflow", "auto");
			$gridTable.tableHeadFixer({
				left: colsLeft,
				right: colsRight
			});
			gx.runtimeTemplates || (o = $gridTable.css("backgroundColor"), o && $(gridTable + " th").css("backgroundColor", o), $.each($(gridTable + " th"), function (n, colsLeft) {
					var colsRight = $(colsLeft);
					colsRight.css("backgroundColor") && colsRight.css("backgroundColor") != "rgba(0, 0, 0, 0)" || $(colsLeft).css("backgroundColor", "white")
				}));

			gx.fx.obs.addObserver('grid.onafterrender', this, this.freezeColumnsAdvanced.closure(this, [gridName, colsLeft, colsRight, width, height]), {
				single: true
			});

		},
		freezeColumns: function (n, colsLeft, colsRight) {
			return this.freezeColumnsAdvanced(n, colsLeft, 0, colsRight)
		}
	}
})();
